 ...
 * {@link org.apache.hadoop.hbase.client.Result}. These methods work off of the
 * raw bytes preventing the explosion of Result into object form.
 *
 *
 * @since 0.1
 */
public class ResultUtil {
  ...
  /**
   * Return a pointer into a potentially much bigger byte buffer that points to
   * the key of a Result.
   *
   * @param r
   */
  ...
  /**
   * Return a pointer into a potentially much bigger byte buffer that points to
   * the key of a Result. Use offset to return a subset of the key bytes, for
   * example to skip the organization ID embedded in all of our keys.
   *
   * @param r
   * @param offset offset added to start of key and subtracted from key length
   * (to select subset of key bytes)
   */
  ...
  /**
   * Comparator for comparing the keys from two Results in-place, without
   * allocating new byte arrays
   */
