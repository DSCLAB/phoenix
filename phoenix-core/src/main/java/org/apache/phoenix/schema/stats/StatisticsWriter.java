...
public class StatisticsWriter implements Closeable {
   ...
   * @throws IOException if the table cannot be created due to an underlying
   * HTable creation error
   */
  ...
  /**
   * Update a list of statistics for a given region. If the UPDATE STATISTICS
   * <tablename> query is issued then we use Upsert queries to update the table
   * If the region gets splitted or the major compaction happens we update using
   * HTable.put()
   *
   * @param tracker - the statistics tracker
   * @param cfKey - the family for which the stats is getting collected.
   * @param mutations - list of mutations that collects all the mutations to
   * commit in a batch
   * @throws IOException if we fail to do any of the puts. Any single failure
   * will prevent any future attempts for the remaining list of stats to update
   */
