 ...
 * {@link org.apache.phoenix.schema.types.PDataType}. Optimized for positional
 * access by index.
 *
 *
 * @since 0.1
...
public abstract class ValueSchema implements Writable {
  ...
  /**
   * @return true if {@link #getEstimatedValueLength()} returns the maximum
   * length of a serialized value for this schema
   */
  ...
  /**
   * Non-nullable fields packed to the left so that we do not need to store
   * trailing nulls. Knowing the minimum position of a nullable field enables
   * this.
   *
  ...
  public boolean equals(Object obj) {
    ...
    if (minNullable != other.minNullable) {
      return false;
    }
