...
cp $(find . -iname phoenix-server-$PHOENIX.jar) $DIR_REL_BIN_PATH;
cp $(find . -iname phoenix-server-$PHOENIX-tests.jar) $DIR_REL_BIN_PATH;
cp $(find . -iname phoenix-server-client-$PHOENIX.jar) $DIR_REL_BIN_PATH;
cp $(find . -iname phoenix-server-$PHOENIX-runnable.jar) $DIR_REL_BIN_PATH;
cp $(find . -iname phoenix-$PHOENIX-thin-client.jar) $DIR_REL_BIN_PATH;
cp $(find . -iname phoenix-spark-$PHOENIX.jar) $DIR_REL_BIN_PATH;
cp $(find . -iname phoenix-spark-$PHOENIX-tests.jar) $DIR_REL_BIN_PATH;
# Copy bin
cp bin/* $DIR_BIN;
...
function_sign() {
  ...
  gpg2 --print-md MD5 $phoenix_tar > $phoenix_tar.md5
  gpg2 --print-md SHA512  $phoenix_tar > $phoenix_tar.sha;
  gpg2 --print-md SHA256  $phoenix_tar >> $phoenix_tar.sha;
}
