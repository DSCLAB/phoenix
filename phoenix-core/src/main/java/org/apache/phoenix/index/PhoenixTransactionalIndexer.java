 ...
 * single coprocessor. Since the transaction manager essentially time orders
 * writes through conflict detection, the logic to maintain a secondary index is
 * quite a bit simpler than the non transactional case. For example, there's no
 * need to muck with the WAL, as failure scenarios are handled by aborting the
 * transaction.
 */
