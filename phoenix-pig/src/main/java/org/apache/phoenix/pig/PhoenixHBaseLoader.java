 ...
 *
 * Example usage: a) TABLE i) A = load 'hbase://table/HIRES' using
 ...
 *
 * b) QUERY i) B = load 'hbase://query/SELECT fname, lname FROM HIRES' using
 * org.apache.phoenix.pig.PhoenixHBaseLoader('localhost');
 *
...
public final class PhoenixHBaseLoader extends LoadFunc implements LoadMetadata {
  ...
  /**
   * Initialize PhoenixPigConfiguration if it is null. Called by
   * {@link #setLocation} and {@link #getSchema}
   *
   * @param location
   * @param configuration
   * @throws PigException
