...
public abstract class AbstractBulkLoadTool extends Configured implements Tool {
  ...
  /**
   * Submits the jobs to the cluster. Loads the HFiles onto the respective
   * tables.
   *
   * @throws Exception
   */
   ...
   * command line if present, otherwise it is taken from the table description.
   *
   * @param conn connection to Phoenix
   * @param cmdLine supplied command line options
   * @param qualifiedTableName table name (possibly with schema) of the table to
   * be imported
   ...
   * ensure no column family names start with '_', as they'd be ignored leading
   * to problems.
   *
