...
public class Tracing {
  ...
  /**
   * Manage the types of frequencies that we support. By default, we never turn
   * on tracing.
   */
  ...
  /**
   * Start a span with the currently configured sampling frequency. Creates a
   * new 'current' span on this thread - the previous 'current' span will be
   * replaced with this newly created span.
   * <p>
   * Hands back the direct span as you shouldn't be detaching the span - use
   * {@link TraceRunnable} instead to detach a span from this operation.
   *
  ...
  public static String getSpanName(Span span) {
    ...
    for (long id : span.getParents()) {
      parentId.add(id);
    }
    return Tracing.TRACE_METRIC_PREFIX + span.getTraceId() + SEPARATOR + parentId
   ...
   * @param description description of the operation being run. If <tt>null</tt>
   * uses the current thread name
   ...
   * used in conjuction with {@link CallRunner#run}.
   * <p>
   * This will always attempt start a new span (which will always start, unless
   * the {@link Sampler} says it shouldn't be traced). If you are just looking
   * for flexible tracing that only turns on if the current thread/query is
   * already tracing, use {@link #wrap(Callable, String)} or
  ...
  /**
   * Add the phoenix span receiver so we can log the traces. We have a single
   * trace source for the whole JVM
   */
