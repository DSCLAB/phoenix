...
public class PhoenixStorageHandlerUtil {
  ...
  public static String getTargetTableName(Table table) {
    ...
    String tableName = tableParameterMap.get(PhoenixStorageHandlerConstants.PHOENIX_TABLE_NAME);
    if (tableName == null) {
  ...
  public static Object[] toTypedValues(JobConf jobConf, String typeName, String[] values) throws
    ...
    for (int i = 0, limit = values.length; i < limit; i++) {
      if (serdeConstants.STRING_TYPE_NAME.equals(typeName)) {
        ...
        String timestampFormat = jobConf.get(PhoenixStorageHandlerConstants.HBASE_TIMESTAMP_FORMAT, PhoenixStorageHandlerConstants.DEFAULT_TIMESTAMP_FORMAT);
  ...
  public static Map<String, String> createColumnTypeMap(JobConf jobConf) {
    ...
    String[] columnNames = jobConf.get(serdeConstants.LIST_COLUMNS).split(PhoenixStorageHandlerConstants.COMMA);
    String[] columnTypes = jobConf.get(serdeConstants.LIST_COLUMN_TYPES).split(PhoenixStorageHandlerConstants.COMMA);
  ...
  public static boolean isTransactionalTable(Properties tableProperties) {
    String tableIsTransactional = tableProperties.getProperty(hive_metastoreConstants.TABLE_IS_TRANSACTIONAL);
  ...
  public static String getOptionsValue(Options options) {
    ...
    content.append("bucket : ").append(bucket).append(", inspectorInfo : ").append(inspectorInfo).append(", minTxnId : ").append(minTxnId).append(", maxTxnId : ")
