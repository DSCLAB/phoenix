...
abstract public class BaseScannerRegionObserver extends BaseRegionObserver {
   ...
   * (later). Custom annotations are used to augment log lines emitted by
   * Phoenix. See https://issues.apache.org/jira/browse/PHOENIX-1198.
   */
   ...
   * that ensures no non IOException is thrown, to prevent the coprocessor from
   ...
   * bugs) and re-throws as DoNotRetryIOException to prevent needless retrying
   * hanging the query for 30 seconds. Unfortunately, until HBASE-7481 gets
   * fixed, there's no way to do the same from a custom filter.
   *
  ...
  protected RegionScanner getWrappedScanner(final ObserverContext<RegionCoprocessorEnvironment> c,
    ...
    return new RegionScanner() {
      ...
      /**
       * When there is a merge in progress while scanning local indexes we might
       * get the key values less than scan start row. In that case we need to
       * scan until get the row key more or equal to scan start key. TODO try to
       * fix this case in LocalIndexStoreFileScanner when there is a merge.
       */
