...
/**
 * Do the actual work of writing to the index tables. Ensures that if we do fail
 * to write to the index table that we cleanly kill the region/server to ensure
...
public class IndexWriter implements Stoppable {
  ...
  /**
   * @throws IOException if the {@link IndexWriter} or
   * {@link IndexFailurePolicy} cannot be instantiated
   */
  ...
  /**
   * Create an {@link IndexWriter} with an already setup {@link IndexCommitter}
   * and {@link IndexFailurePolicy}.
   *
   ...
   * <p>
   * This method is blocking and could potentially cause the writer to block for
   * a long time as we write the index updates. When we return depends on the
   * specified {@link IndexCommitter}.
   * <p>
   * If update fails, we pass along the failure to the installed
   * {@link IndexFailurePolicy}, which then decides how to handle the failure.
   * By default, we use a {@link KillServerOnFailurePolicy}, which ensures that
   * the server crashes when an index write fails, ensuring that we get WAL
   ...
   * <p>
   * This method is blocking and could potentially cause the writer to block for
   * a long time as we write the index updates. We only return when either:
   * <ol>
   * <li>All index writes have returned, OR</li>
   * <li>Any single index write has failed</li>
   * </ol>
   * We attempt to quickly determine if any write has failed and not write to
   * the remaining indexes to ensure a timely recovery of the failed index
   * writes.
   *
   * @param toWrite Updates to write
   * @throws IndexWriteException if we cannot successfully write to the index.
   * Whether or not we stop early depends on the {@link IndexCommitter}.
   */
