...
public class KeyRange implements Writable {
  ...
  /**
   * KeyRange for variable length null values. Since we need to represent this
   * using an empty byte array (which is what we use for upper/lower bound), we
   * create this range using the private constructor rather than going through
   * the static creation method (where this would not be possible).
   */
  public static final KeyRange IS_NULL_RANGE = new KeyRange(ByteUtil.EMPTY_BYTE_ARRAY, true, ByteUtil.EMPTY_BYTE_ARRAY, true);
  /**
   * KeyRange for non null variable length values. Since we need to represent
   * this using an empty byte array (which is what we use for upper/lower
   * bound), we create this range using the private constructor rather than
   ...
   * @param comparator comparator used to do compare the byte array using offset
   * and length
   * @return -1 if the lower bound is less than the upper bound, 1 if the lower
   * bound is greater than the upper bound, and 0 if they are equal.
   */
