 ...
 * methods are supported: - {@link #executeQuery(String)}
 * - {@link #executeUpdate(String)}
 * - {@link #execute(String)}
 * - {@link #getResultSet()}
 * - {@link #getUpdateCount()}
 * - {@link #close()} The Statement only supports the following options: -
...
public class PhoenixStatement implements Statement, SQLCloseable {
  ...
  /**
   * Execute the current batch of statements. If any exception occurs during
   * execution, a {@link org.apache.phoenix.exception.BatchUpdateException} is
   * thrown which includes the index of the statement within the batch when the
   * exception occurred.
   */
  @Override
  ...
  @Override
  /**
   * When setting the query timeout via JDBC timeouts must be expressed in
   * seconds. Therefore we need to convert the default timeout to milliseconds
   * for internal use.
   */
  ...
  @Override
  /**
   * When getting the query timeout via JDBC timeouts must be expressed in
   * seconds. Therefore we need to convert the default millisecond timeout to
   * seconds.
   */
  ...
  /**
   * Returns the configured timeout in milliseconds. This internally enables the
   * of use millisecond timeout granularity and honors the exact value
   * configured by phoenix.query.timeoutMs.
   */
