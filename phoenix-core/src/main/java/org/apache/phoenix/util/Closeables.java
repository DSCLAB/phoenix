...
public class Closeables {
  ...
  /**
   * Close a {@code Closeable}, returning an {@code IOException} if it occurs
   * while closing instead of throwing it. This is nearly a clone of the Guava
   * Closeables.closeQuietly method which has long since been removed from
   * Guava.
   *
   * Use of this method should be avoided -- quietly swallowing IOExceptions
   * (particularly on Closeables that are being written to) is a code smell. Use
   * of the equivalent method in Guava was done for this reason.
   *
   ...
   *
   * If any of the close's fail with an IOException, those exception(s) will be
   * thrown after attempting to close all of the inputs.
   */
