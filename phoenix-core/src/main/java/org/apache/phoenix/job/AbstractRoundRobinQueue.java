 ...
 * elements on per-producer basis and iterates through each producer queue in
 * round robin fashion.
 *
 */
public abstract class AbstractRoundRobinQueue<E> extends AbstractQueue<E>
  ...
  /**
   * Construct an AbstractBlockingRoundRobinQueue that limits the size of the
   * queued elements to at most maxSize. Attempts to insert new elements after
   * that point will cause the caller to block.
   *
   * @param maxSize
   */
  ...
  /**
   * @param newProducerToFront If true, new producers go to the front of the
   * round-robin list, if false, they go to the end.
   */
  ...
  /**
   * Implementations must extracts the producer object which is used as the key
   * to identify a unique producer.
   */
