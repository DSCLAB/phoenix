...
/**
 * Scan the primary table. This is similar to HBase's scanner, but ensures that
 * you will never see deleted columns/rows
 */
public interface Scanner extends Closeable {
   ...
   * {@link KeyValue} is present in <tt>this</tt>, it will be returned by the
   * next call to {@link #next()}. Otherwise, returns the next {@link KeyValue}
   * after the seeked {@link KeyValue}.
   *
   * @param next {@link KeyValue} to seek to. Doesn't need to already be present
   * in <tt>this</tt>
  ...
  /**
   * Read the {@link KeyValue} at the top of <tt>this</tt> without 'popping' it
   * off the top of the scanner.
   *
