...
public class JobManager<T> extends AbstractRoundRobinQueue<T> {
  ...
  /**
   * Subclasses FutureTask for the sole purpose of providing
   * {@link #getCallable()}, which is used to extract the producer in the
   * {@link JobBasedRoundRobinQueue}
   */
  ...
  /**
   * Instrumented version of {@link JobFutureTask} that measures time spent by a
   * task at various stages in the queue and when executed.
   */
  ...
  /**
   * Delegating callable implementation that preserves the parentJobId and sets
   * up thread tracker stuff before delegating to the actual command.
   */
  ...
  /**
   * Extension of the default thread factory returned by
   * {@code Executors.defaultThreadFactory} that sets the context classloader on
   * newly-created threads to be a specific classloader (and not the context
   * classloader of the calling thread).
   * <p/>
   * See {@link org.apache.phoenix.util.PhoenixContextExecutor} for the
   * rationale on changing the context classloader.
   */
  ...
  /**
   * Thread pool executor that instruments the various characteristics of the
   * backing pool of threads and queue. This executor assumes that all the tasks
   * handled are of type {@link JobManager.InstrumentedJobFutureTask}
   */
