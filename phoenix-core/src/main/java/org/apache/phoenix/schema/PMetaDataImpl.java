...
/**
 *
 * Client-side cache of MetaData. Not thread safe, but meant to be used in a
 * copy-on-write fashion. Internally uses a LinkedHashMap that evicts the oldest
 * entries when size grows beyond the maxSize specified at create time.
 *
 */
public class PMetaDataImpl implements PMetaData {
  ...
  private static class PMetaDataCache implements Cloneable {
    ...
    /**
     * Used when the cache is growing past its max size to clone in a single
     * pass. Removes least recently used tables to get size of cache below its
     * max size by the overage amount.
     */
