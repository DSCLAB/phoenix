...
/**
 * Combine a simplified version of the logic in the ScanQueryMatcher and the
 * KeyValueScanner. We can get away with this here because we are only concerned
 * with a single MemStore for the index; we don't need to worry about multiple
 * column families or minimizing seeking through file - we just want to iterate
 * the kvs quickly, in-memory.
 */
public class FilteredKeyValueScanner implements KeyValueScanner {
  ...
  /**
   * Same a {@link KeyValueScanner#next()} except that we filter out the next
   * {@link KeyValue} until we find one that passes the filter.
   *
   * @return the next {@link KeyValue} or <tt>null</tt> if no next
   * {@link KeyValue} is present and passes all the filters.
   */
  @Override
