...
/**
 *
 * Schema for the bytes in a RowKey. For the RowKey, we use a null byte to
 * terminate a variable length type, while for KeyValue bytes we write the
 * length of the var char preceding the value. We can't do that for a RowKey
 * because it would affect the sort order.
 *
 *
 * @since 0.1
 */
public class RowKeySchema extends ValueSchema {
  ...
  /**
   * Move the bytes ptr to the next position in the row key relative to its
   * current position. You must have a complete row key. Use @link
   * {@link #position(ImmutableBytesWritable, int, int)} if you have a partial
   * row key.
   *
   ...
   * @return true if a value was found and ptr was set, false if the value is
   * null and ptr was not set, and null if the value is null and there are no
   * more values
   */
  ...
  private Boolean next(ImmutableBytesWritable ptr, int position, int maxOffset, boolean isFirst) {
    ...
    } else if (position + 1 == getFieldCount()) { // Last field has no terminator
  ...
  /**
   * Like
   * {@link #next(org.apache.hadoop.hbase.io.ImmutableBytesWritable, int, int)},
   * but also includes the next {@code extraSpan} additional fields in the bytes
   * ptr. This allows multiple fields to be treated as one concatenated whole.
   *
   * @param ptr bytes pointer pointing to the value at the positional index
   * provided.
   * @param position zero-based index of the next field in the value schema
  ...
  /**
   * Like
   * {@link #reposition(org.apache.hadoop.hbase.io.ImmutableBytesWritable, int, int, int, int)},
   * but also includes the next {@code extraSpan} additional fields in the bytes
   * ptr. This allows multiple fields to be treated as one concatenated whole.
   *
   * @param extraSpan the number of extra fields to expand the ptr to contain.
  ...
  /**
   * Extends the boundaries of the {@code ptr} to contain the next
   * {@code extraSpan} fields in the row key.
   *
   * @param ptr bytes pointer pointing to the value at the positional index
   * provided.
