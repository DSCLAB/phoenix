...
/**
 *
 * Base class for aggregate functions that calculate an aggregation using a
 * single {{@link Aggregator}
 *
 *
 * @since 0.1
 */
abstract public class SingleAggregateFunction extends AggregateFunction {
  ...
  /**
   * Sort aggregate functions with nullable fields last. This allows us not to
   * have to store trailing null values. Within non-nullable/nullable groups,
   * put fixed width values first since we can access those more efficiently
   * (i.e. we can skip over groups of them in-mass instead of reading the length
   * of each one to skip over as required by a variable length value).
   */
  ...
  /**
   * Create the aggregator to do server-side aggregation. The data type of the
   * returned Aggregator must match the data type returned by
   * {@link #newClientAggregator()}
   *
  ...
  /**
   * Create the aggregator to do client-side aggregation based on the results
   * returned from the aggregating coprocessor. The data type of the returned
   * Aggregator must match the data type returned by
   * {@link #newServerAggregator(Configuration)}
   *
