...
public class MutationState implements SQLCloseable {
   ...
   * newer one will take precedence.
   *
    ...
    for (Map.Entry<TableRef, Map<ImmutableBytesPtr, RowMutationState>> entry : newMutation.mutations.entrySet()) {
      ...
      if (existingRows != null) { // Rows for that table already exist
        ...
        for (Map.Entry<ImmutableBytesPtr, RowMutationState> rowEntry : entry.getValue().entrySet()) {
          ...
          } else if (!isIndex) { // Don't count index rows in row count
            numRows++;
          }
        }
      ...
      while (mutationsIterator.hasNext()) {
        ...
        do {
          ...
          try {
            ...
            shouldRetry = false;
            if (logger.isDebugEnabled()) {
              logger.debug(LogUtil.addCustomAnnotations("Total time for batch call of  " + mutations.size() + " mutations into " + table.getName().getString() + ": " + duration + " ms", connection));
            }
          } catch (Exception e) {
