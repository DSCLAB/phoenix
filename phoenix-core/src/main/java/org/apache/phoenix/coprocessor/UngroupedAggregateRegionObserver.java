...
public class UngroupedAggregateRegionObserver extends BaseScannerRegionObserver {
  ...
  /**
   * This lock used for synchronizing the state of null   {@link UngroupedAggregateRegionObserver#scansReferenceCount},
     * {@link UngroupedAggregateRegionObserver#isRegionClosing} variables used to
   * avoid possible dead lock situation in case below steps: 1. We get read lock
   * when we start writing local indexes, deletes etc.. 2. when memstore reach
   * threshold, flushes happen. Since they use read (shared) lock they happen
   * without any problem until someone tries to obtain write lock. 3. at one
   * moment we decide to split/bulkload/close and try to acquire write lock. 4.
   * Since that moment all attempts to get read lock will be blocked. I.e. no
   * more flushes will happen. But we continue to fill memstore with local index
   * batches and finally we get RTBE.
   *
   * The solution to this is to not allow or delay operations acquire the write
   * lock. 1) In case of split we just throw IOException so split won't happen
   * but it will not cause any harm. 2) In case of bulkload failing it by
   * throwing the exception. 3) In case of region close by balancer/move wait
   * before closing the reason and fail the query which does write after
   * reading.
   *
  ...
  /**
   * To maintain the number of scans used for create index, delete and upsert
   * select operations which reads and writes to same region in coprocessors.
   */
  @GuardedBy("lock")
   ...
   * balancer/move/merge. In this case if the memstore size reaches
   * blockingMemstoreSize better to fail query because there is a high chance
   * that flush might not proceed and memstore won't be freed up.
   *
   * @throws IOException
   */
  ...
  protected RegionScanner doPostScannerOpen(final ObserverContext<RegionCoprocessorEnvironment> c, final Scan scan, final RegionScanner s) throws IOException {
     ...
     * hbase.hregion.memstore.flush.size bytes. This avoids flush storm to hdfs
     * for cases like index building where reads and write happen to all the
     * table regions in the server.
     */
  ...
  /**
   *
   * Callable to encapsulate the collection of stats triggered by UPDATE
   * STATISTICS command.
   *
