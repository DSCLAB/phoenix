...
/**
 * Determines if the natural key order of the rows returned by the scan will
 * match the order of the expressions. For GROUP BY, if order is preserved we
 * can use an optimization during server-side aggregation to do the aggregation
 * on-the-fly versus keeping track of each distinct group. We can only do this
 * optimization if all the rows for each group will be contiguous. For ORDER BY,
 * we can drop the ORDER BY statement if the order is preserved.
 *
 */
public class OrderPreservingTracker {
  ...
  /**
   *
   * Determines if an expression is held constant. Only works for columns in the
   * PK currently, as we only track whether PK columns are held constant.
   *
   */
  ...
  /**
   *
   * Visitor used to determine if order is preserved across a list of
   * expressions (GROUP BY or ORDER BY expressions)
   *
   */
