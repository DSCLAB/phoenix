 ...
 * key column.</p>
 * The {@link ColumnModifier#apply apply} method is called when the bytes for a
 * specific column are first written to HBase and again when they are read back.
 * Phoenix attemps to minimize calls to apply when bytes are read out of HBase.
 *
 *
 * @since 1.2
 */
public enum ColumnModifier {
  /**
   * Invert the bytes in the src byte array to support descending ordering of
   * row keys.
   */
  SORT_DESC(1) {
    @Override
  ...
  /**
   * Copies the bytes from source array to destination array and applies the
   * column modifier operation on the bytes starting at the specified offsets.
   * The column modifier is applied to the number of bytes matching the
   * specified length.
   *
   ...
   * @param dest the destination byte array into which to transfer the modified
   * bytes.
   * @param dstOffset the offset into the destination byte array at which to
   * begin
  ...
  /**
   * Copies the bytes from source array to a newly allocated destination array
   * and applies the column modifier operation on the bytes starting at the
   * specified offsets. The column modifier is applied to the number of bytes
