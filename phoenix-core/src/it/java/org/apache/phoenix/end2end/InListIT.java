...
public class InListIT extends BaseHBaseManagedTimeIT {
   ...
   * properties. Assumes 5 pk columns of the given type. Adds a non primary key
   * column named "nonPk"
   *
   ...
   * table is multi-tenant, also creates a tenant view for that table and
   * returns the name of the view instead.
   *
   * @param baseConn a non-tenant specific connection. Used to create the base
   * tables
   ...
   * @param saltBuckets the number of salt buckets if the table is salted,
   * otherwise 0
   * @return the table or view name that should be used to access the created
   * table
   */
  ...
  /**
   * Tests the given where clause against the given upserts by comparing against
   * the list of expected result strings.
   *
   * @param upsertBodies list of upsert bodies with the form "(pk1, pk2, ...,
   * nonPk) VALUES (1, 7, ..., "row1") excludes the "UPSERT INTO table_name "
   * segment so that table name can vary
   * @param whereClause the where clause to test. Should only refer to the pks
   * upserted.
