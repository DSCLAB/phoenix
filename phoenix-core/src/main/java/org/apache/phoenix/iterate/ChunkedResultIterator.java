...
/**
 * {@code PeekingResultIterator} implementation that loads data in chunks. This
 * is intended for basic scan plans, to avoid loading large quantities of data
 * from HBase in one go.
 */
public class ChunkedResultIterator implements PeekingResultIterator {
  ...
  public static class ChunkedResultIteratorFactory implements ParallelIteratorFactory {
    ...
    public PeekingResultIterator newIterator(StatementContext context, ResultIterator scanner, Scan scan) throws SQLException {
      ...
      if (logger.isDebugEnabled()) {
        logger.debug(LogUtil.addCustomAnnotations("ChunkedResultIteratorFactory.newIterator over " + tableRef.getTable().getName().getString() + " with " + scan, ScanUtil.getCustomAnnotations(scan)));
      }
  ...
  private PeekingResultIterator getResultIterator() throws SQLException {
    if (resultIterator.peek() == null && lastKey != null) {
      ...
      if (logger.isDebugEnabled()) {
        logger.debug(LogUtil.addCustomAnnotations("Get next chunked result iterator over " + tableRef.getTable().getName().getString() + " with " + scan, ScanUtil.getCustomAnnotations(scan)));
      }
      ResultIterator singleChunkResultIterator = new SingleChunkResultIterator(
              new TableResultIterator(context, tableRef, scan), chunkSize);
      resultIterator = delegateIteratorFactory.newIterator(context, singleChunkResultIterator, scan);
  ...
  /**
   * ResultIterator that runs over a single chunk of results (i.e. a portion of
   * a scan).
   */
  private class SingleChunkResultIterator implements ResultIterator {
    ...
    /**
     * Prefix region start key to last key to form actual row key in case of
     * local index scan.
     */
