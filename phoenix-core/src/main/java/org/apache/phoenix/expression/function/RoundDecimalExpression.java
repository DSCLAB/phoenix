...
public class RoundDecimalExpression extends ScalarFunction {
  ...
  /**
   * Creates a {@link RoundDecimalExpression} with rounding scale given by
   *
   * @param scale.
   *
   */
  ...
  /**
   * Creates a {@link RoundDecimalExpression} with a default scale of 0 used for
   * rounding.
   *
   */
  ...
  public KeyPart newKeyPart(final KeyPart childPart) {
    return new KeyPart() {
      ...
      /**
       * Produces a the given decimal rounded to this rounding expression's
       * scale. If the decimal requires more scale precision to produce than
       * this expression has, as in ROUND(?, 2) &gt; 2.0098974, it ensures that
       * the decimal is rounded such that the given operator will still produce
       * correct results.
       *
       * @param decimal the decimal to round with this expression's scale
       * @param op the operator to preserve comparison with in the event of lost
       * precision
   ...
   * this rounding expression. For example, a ROUND expression with scale 2 will
   * produce the result "2.05" with any decimal in the range [2.045, 2.0545).
   ...
   * @return a KeyRange of DECIMAL keys that can be rounded by this expression
   * to produce result
   * @throws IllegalArgumentException if the result has more scale than this
  ...
  /**
   * Determines whether this rounding expression's scale has enough precision to
   * produce the minimum precision for the input decimal. In other words,
   * determines whether the given decimal can be rounded to this scale without
   * losing ordering information. For example, an expression with a scale of 2
   * has enough precision to produce "2.3", "2.71" and "2.100000", but does not
   * have enough precision to produce "2.001"
   *
   * @param result the decimal to round
   * @return true if the given decimal can be precisely matched by this rounding
   * expression
   */
  ...
  /**
   * Returns the given decimal rounded to this rounding expression's scale. For
   * example, with scale 2 the decimal "2.453" would be rounded to either 2.45
   * or 2.46 depending on the rounding mode, while "2.38" and "2.7" would be
   * unchanged.
   *
  ...
  /**
   * Produces a value half of a "step" forward in this expression's rounding
   * scale. For example with a scale of 2, "2.5" would be stepped forward to
   * "2.505".
   */
  ...
  /**
   * Produces a value one "step" back in this expression's rounding scale. For
   * example with a scale of 2, "2.5" would be stepped back to "2.49".
   */
