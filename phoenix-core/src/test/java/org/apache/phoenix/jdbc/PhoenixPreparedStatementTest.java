...
public class PhoenixPreparedStatementTest extends BaseConnectionlessQueryTest {
   ...
   * stmt.setQueryTimeout() JDBC method, we correctly store the timeout in both
  ...
  @Test
  /**
   * Validates if a user sets the timeout to zero that we store the timeout in
   * millis as the Integer.MAX_VALUE.
   */
  ...
  @Test
  /**
   * Validates that setting custom phoenix query timeout using the
   * phoenix.query.timeoutMs config property is honored.
   */
