 ...
 * Puts/Delets are passed to an {@link IndexBuilder} to determine the actual
 * updates to make.
 * <p>
 * If the WAL is enabled, these updates are then added to the WALEdit and
 * attempted to be written to the WAL after the WALEdit has been saved. If any
 * of the index updates fail, this server is immediately terminated and we rely
 * on WAL replay to attempt the index updates again (see
 * {@link #preWALRestore(ObserverContext, HRegionInfo, HLogKey, WALEdit)}).
 * <p>
 * If the WAL is disabled, the updates are attempted immediately. No consistency
 * guarantees are made if the WAL is disabled - some or none of the index
 * updates may be successful. All updates in a single batch must have the same
 * durability level - either everything gets written to the WAL or nothing does.
 * Currently, we do not support mixed-durability updates within a single batch.
 * If you want to have different durability levels, you only need to split the
 * updates into two different batches.
 */
public class Indexer extends BaseRegionObserver {
   ...
   * running. Generally, you only want to ignore this for testing or for custom
   * versions of HBase.
   */
   ...
   * recovery mechanisms more robust in the face of recoverying index regions
   * that were on the same server as the primary table region
   */
  private PerRegionIndexWriteCache failedIndexEdits = new PerRegionIndexWriteCache();

  /**
   * IndexWriter for writing the recovered index edits. Separate from the main
   * indexer since we need different write/failure policies
   */
  ...
  /**
   * Create a custom {@link InternalScanner} for a compaction that tracks the
   * versions of rows that are removed so we can clean then up from the the
   * index table(s).
   * <p>
   * This is not yet implemented - its not clear if we should even mess around
   * with the Index table for these rows as those points still existed. TODO: v2
   * of indexing
   */
  @Override
   ...
   *
   * @param desc {@link HTableDescriptor} for the table on which indexing should
   * be enabled
   * @param builder class to use when building the index for this table
   * @param properties map of custom configuration options to make available to
   * your {@link IndexBuilder} on the server-side
   * @param priority TODO
