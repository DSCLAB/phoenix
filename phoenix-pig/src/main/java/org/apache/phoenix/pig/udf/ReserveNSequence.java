 ...
 *
 * @note The way this UDF is invoked we open a new connection for every tuple
 * row. The UDF will not perform well on large datasets as it involves creating
 * a new connection for every tuple row
 */
public class ReserveNSequence extends EvalFunc<Long> {
  ...
  /**
   * Reserve N next sequences for a sequence name. N is the first field in the
   * tuple. Sequence name is the second field in the tuple zkquorum is the third
   * field in the tuple
   */
  @Override
  ...
  /**
   * Cleanup to be performed at the end. Close connection
   */
  @Override
  public void finish() {
    if (connection != null) {
