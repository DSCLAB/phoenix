...
/**
 * Utility code to do optimized byte-array comparison. This is borrowed from
 * org.apache.hadoop.io.FastByteComparisons which was borrowed and slightly
 * modified from Guava's {@link UnsignedBytes} class to be able to compare
 * arrays that start at non-zero offsets.
 *
 * The only difference is that we sort a smaller length bytes as *larger* than
 * longer length bytes when all the bytes are the same.
 */
