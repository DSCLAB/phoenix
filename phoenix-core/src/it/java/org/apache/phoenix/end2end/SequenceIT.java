...
public class SequenceIT extends BaseClientManagedTimeIT {
  ...
  @Test
  /**
   * Test to validate that the bug discovered in PHOENIX-2149 has been fixed.
   * There was an issue whereby, when closing connections and returning
   * sequences we were not setting the limit reached flag correctly and this was
   * causing the max value to be ignored as the LIMIT_REACHED_FLAG value was
   * being unset from true to false.
   */
  ...
  /**
   * Helper to verify the sequence values returned in multiple ResultSets each
   * containing one row
   *
