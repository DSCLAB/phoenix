...
public class ExpressionCompiler extends UnsupportedAllParseNodeVisitor<Expression> {
  ...
  /**
   * Add expression to the expression manager, returning the same one if already
   * used.
   */
   ...
   * @param children the child expression arguments to the function expression
   * node.
   */
  ...
  /**
   * Called by visitor to resolve a column expression node into a column
   * reference. Derived classes may use this as a hook to trap all column
   * resolves.
   *
   * @param node a column expression node
   * @return a resolved ColumnRef
   * @throws SQLException if the column expression node does not refer to a
   * known/unambiguous column
   */
  ...
  public Expression visitLeave(SubtractParseNode node, List<Expression> children) throws SQLException {
    return visitLeave(node, children, new ArithmeticExpressionBinder() {
      ...
      public Expression create(ArithmeticParseNode node,
        ...
        /**
         * For date1-date2, we want to coerce to a LONG because this cannot be
         * compared against another date. It has essentially become a number.
         * For date1-5, we want to preserve the DATE type because this can still
         * be compared against another date and cannot be multiplied or divided.
  ...
  public Expression visitLeave(DivideParseNode node, List<Expression> children) throws SQLException {
    for (int i = 1; i < children.size(); i++) { // Compile time check for divide by zero and null
      ...
      if (child.getDataType() != null && child instanceof LiteralExpression) {
        ...
        } else if (literal.getDataType().compareTo(literal.getValue(), 0L, PLong.INSTANCE) == 0) {
          throw new SQLExceptionInfo.Builder(SQLExceptionCode.DIVIDE_BY_ZERO).build().buildException();
        }
      }
    }
