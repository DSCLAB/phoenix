...
public class IndexMetaDataCacheClient {
  ...
  /**
   * Construct client used to send index metadata to each region server for
   * caching during batched put for secondary index maintenance.
   *
   * @param connection the client connection
   * @param cacheUsingTableRef table ref to table that will use the cache during
   * its scan
   */
  ...
  /**
   * Determines whether or not to use the IndexMetaDataCache to send the index
   * metadata to the region servers. The alternative is to just set the index
   * metadata as an attribute on the mutations.
   *
   * @param connection
  ...
  /**
   * Send the index metadata cahce to all region servers for regions that will
   * handle the mutations.
   *
   * @return client-side {@link ServerCache} representing the added index
   * metadata cache
   * @throws SQLException
   * @throws MaxServerCacheSizeExceededException if size of hash cache exceeds
   * max allowed size
   */
