 ...
 * Note, all of the sizes here, but especially OBJECT_SIZE and ARRAY_SIZE are
 * estimates and will depend on the JVM itself (and which JVM, 64bit vs. 32bit,
 * etc). The current values are based on: Java HotSpot(TM) 64-Bit Server
 * VM/14.2-b01
 *
 * (Uncomment out and run the main w/ appropriate object to test) Also, see this
 * link:
...
public class SizedUtil {
  ...
  public static long sizeOfMap(int nRows, int keySize, int valueSize) {
    return SizedUtil.OBJECT_SIZE * 4 + sizeOfArrayList(nRows) /* key set */ + nRows * (SizedUtil.MAP_ENTRY_SIZE
            + /* entry set */ keySize
            + // key size
