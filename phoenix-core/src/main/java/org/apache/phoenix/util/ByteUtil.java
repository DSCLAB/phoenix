...
public class ByteUtil {
  ...
  /**
   * Serialize an array of byte arrays into a single byte array. Used to pass
   * through a set of bytes arrays as an attribute of a Scan. Use
   * {@link #toByteArrays(byte[], int)} to convert the serialized byte array
   * back to the array of byte arrays.
   *
  ...
  /**
   * Deserialize a byte array into a set of byte arrays. Used in coprocessor to
   * reconstruct byte arrays from attribute value passed through the Scan.
   *
   * @param b byte array containing serialized byte arrays (created by
   * {@link #toBytes(byte[][])}).
   * @param length number of byte arrays that were serialized
   * @return array of now deserialized byte arrays
   * @throws IllegalStateException if there are more than length number of byte
   * arrays that were serialized
   */
   ...
   * @return newly allocated byte array that is a concatenation of all the byte
   * arrays passed in
   */
  ...
  /**
   * Decode a vint from the buffer pointed at to by ptr and increment the offset
   * of the ptr by the length of the vint.
   *
  ...
  /**
   * Put long as variable length encoded number at the offset in the result byte
   * array
   *
   ...
   * @return a new byte array with the next key or null if the key could not be
   * incremented because it's already at its max value.
   */
  ...
  /**
   * Expand the key to length bytes using the fillByte to fill the bytes beyond
  ...
  /**
   * Given an ImmutableBytesWritable, returns the payload part of the argument
   * as an byte array.
   */
