...
public class IndexManagementUtil {
   ...
   * Generally, this is useful to for an index codec to determine if a given
   * update should even be indexed. This assumes that for any index, there are
   * going to small number of columns, versus the number of kvs in any one
   * batch.
   */
   ...
   * Generally, this is useful to for an index codec to determine if a given
   * update should even be indexed. This assumes that for any index, there are
   * going to small number of kvs, versus the number of columns in any one
   * batch.
   * <p>
   * This employs the same logic as
   * {@link #updateMatchesColumns(Collection, List)}, but is flips the iteration
   * logic to search columns before kvs.
   */
  ...
  /**
   * Propagate the given failure as a generic {@link IOException}, if it isn't
   * already
   *
   * @param e reason indexing failed. If ,tt>null</tt>, throws a
   * {@link NullPointerException}, which should unload the coprocessor.
   */
