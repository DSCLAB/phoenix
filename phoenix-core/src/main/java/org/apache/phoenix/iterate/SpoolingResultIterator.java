 ...
 * threshold has been reached. If the in-memory threshold is not reached, the
 * results are held in memory with no disk writing perfomed.
 *
 * <p>
 * Spooling is deprecated and shouldn't be used while implementing new features.
 * As of HBase 0.98.17, we rely on pacing the server side scanners instead of
 * pulling rows from the server and potentially spooling to a temporary file
 * created on clients.
 * </p>
 *
 * @since 0.1
...
public class SpoolingResultIterator implements PeekingResultIterator {
   ...
   * features. As of HBase 0.98.17, we rely on pacing the server side scanners
   * instead of pulling rows from the server and potentially spooling to a
   * temporary file created on clients.
   */
  @Deprecated
   ...
   * spooling them to disk once a threshold has been reached. The scanner passed
   * in is closed prior to returning.
   *
   * @param scanner the results of a table scan
   * @param mm memory manager tracking memory usage across threads.
   * @param thresholdBytes the requested threshold. Will be dialed down if
   * memory usage (as determined by the memory manager) is exceeded.
   * @throws SQLException
   */
