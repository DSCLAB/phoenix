...
/**
 *
 * Region observer coprocessor for sequence operations: 1) For creating a
 * sequence, as checkAndPut does not allow us to scope the Get done for the
 * check with a TimeRange. 2) For incrementing a sequence, as increment does not
 * a) allow us to set the timestamp of the key value being incremented and b)
 * recognize when the key value being incremented does not exist 3) For deleting
 * a sequence, as checkAndDelete does not allow us to scope the Get done for the
 * check with a TimeRange.
 *
 *
 * @since 3.0.0
 */
public class SequenceRegionObserver extends BaseRegionObserver {
   ...
   * Increment implementation (HBASE-10254): 1) Lack of recognition and
   * identification of when the key value to increment doesn't exist 2) Lack of
   * the ability to set the timestamp of the updated key value. Works the same
   * as existing region.increment(), except assumes there is a single column to
   ...
   *
   * @param key key used while creating KeyValue
   * @param cqBytes column qualifier of KeyValue
  ...
  /**
   * Override the preAppend for checkAndPut and checkAndDelete, as we need the
   * ability to a) set the TimeRange for the Get being done and b) return
   * something back to the client to indicate success/failure
   */
  ...
  /**
   * Determines whether a request for incrementing the sequence was a bulk
   * allocation and if so what the number of slots to allocate is. This is
   * triggered by the NEXT <n> VALUES FOR expression. For backwards
   * compatibility with older clients, we default the value to 1 which preserves
