 ...
 * transactions against the hive metastore,allowing Phoenix metadata to be kept
 * in sync with Hive'smetastore.
 */
public class PhoenixMetaHook implements HiveMetaHook {
  ...
  private Map<String, String> getColumnMappingMap(String columnMappings) {
    ...
    Map<String, String> columnMappingMap = Splitter.on(PhoenixStorageHandlerConstants.COMMA)
            .trimResults().withKeyValueSeparator(PhoenixStorageHandlerConstants.COLON).split(columnMappings);
