...
public class SkipScanFilter extends FilterBase implements Writable {
  ...
  /**
   * We know that initially the first row will be positioned at or after the
   * first possible key.
   */
  ...
  /**
   * Intersect the ranges of this filter with the ranges form by lowerInclusive
   * and upperInclusive key and filter out the ones that are not included in the
   * region. Return the new intersected SkipScanFilter or null if there is no
   * intersection.
   */
