...
/**
 *
 * Class that creates a new select statement ensuring that a literal always
 * occurs on the RHS (i.e. if literal found on the LHS, then the operator is
 * reversed and the literal is put on the RHS)
 *
 *
 * @since 0.1
 */
public class StatementNormalizer extends ParseNodeRewriter {
  ...
  /**
   * Rewrite the select statement by switching any constants to the right hand
