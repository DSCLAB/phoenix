...
/**
 *
 * JDBC Driver implementation of Phoenix for production. To use this driver,
 * specify the following URL: jdbc:phoenix:<zookeeper quorum server name>; Only
 * an embedded driver is currently supported (Phoenix client runs in the same
 * JVM as the driver). Connections are lightweight and are not pooled. The last
 * part of the URL, the hbase zookeeper quorum server name, determines the hbase
 * cluster to which queries will be routed.
 *
 *
 * @since 0.1
 */
