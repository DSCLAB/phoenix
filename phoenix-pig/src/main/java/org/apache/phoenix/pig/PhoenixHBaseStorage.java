 ...
 *
 * Alternative usage: A = load 'testdata' as (a:chararray, b:chararray, e:
 * datetime); STORE A into 'hbase://CORE.ENTITY_HISTORY/ID,F.B,F.E' using
 * org.apache.bdaas.PhoenixHBaseStorage('localhost','-batchSize 5000');
 *
 * The above reads a file 'testdata' and writes the elements ID, F.B, and F.E to
 * HBase. In this example, ID is the row key, and F is the column family for the
 * data elements. First argument to this StoreFunc is the server, the 2nd
 * argument is the batch size for upserts via Phoenix. In this case, less than
 * the full table row is required. For configuration message, look in the info
 * log file.
 *
