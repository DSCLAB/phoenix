...
/**
 * Test that we correctly fail for versions of HBase that don't support current
 * properties
 */
...
public class FailForUnsupportedHBaseVersionsIT {
  ...
  /**
   * We don't support WAL Compression for HBase &lt; 0.94.9, so we shouldn't
   * even allow the server to start if both indexing and WAL Compression are
   * enabled for the wrong versions.
   */
  @Test
  ...
  /**
   * Test that we correctly abort a RegionServer when we run tests with an
   * unsupported HBase version. The 'completeness' of this test requires that we
   * run the test with both a version of HBase that wouldn't be supported with
   * WAL Compression. Currently, this is the default version (0.94.4) so just
   * running 'mvn test' will run the full test. However, this test will not fail
   * when running against a version of HBase with WALCompression enabled.
   * Therefore, to fully test this functionality, we need to run the test
   * against both a supported and an unsupported version of HBase (as long as we
   * want to support an version of HBase that doesn't support custom WAL
   * Codecs).
   *
