...
public abstract class PhoenixEmbeddedDriver implements Driver, SQLCloseable {
  ...
  /**
   * Get or create if necessary a QueryServices that is associated with the
   * HBase zookeeper quorum name (part of the connection URL). This will cause
   * the underlying Configuration held by the QueryServices to be shared for all
   * connections to the same HBase cluster.
   *
  ...
  public static class ConnectionInfo {
    ...
    /**
     * Detect url with quorum:1,quorum:2 as HBase does not handle different port
     * numbers for different quorum hostnames.
     *
     * @param portStr
     * @return
     */
    ...
    /**
     * Constructs a Configuration object to use when performing a Kerberos
     * login.
     *
    ...
    public boolean equals(Object obj) {
      ...
      } else if (!zookeeperQuorum.equals(other.zookeeperQuorum)) {
        return false;
      }
      if (port == null) {
        if (other.port != null) {
