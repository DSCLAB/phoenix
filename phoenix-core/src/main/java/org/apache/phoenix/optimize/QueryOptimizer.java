...
public class QueryOptimizer {
  ...
  /**
   * Order the plans among all the possible ones from best to worst. Since we
   * don't keep stats yet, we use the following simple algorithm: 1) If the
   * query is a point lookup (i.e. we have a set of exact row keys), choose that
   * one immediately. 2) If the query has an ORDER BY and a LIMIT, choose the
   * plan that has all the ORDER BY expression in the same order as the row key
   * columns. 3) If there are more than one plan that meets (1&2), choose the
   * plan with: a) the most row key columns that may be used to form the
   * start/stop scan key (i.e. bound slots). b) the plan that preserves ordering
   * for a group by. c) the non local index table plan TODO: We should make more
   * of a cost based choice: The largest number of bound slots does not
   * necessarily correspond to the least bytes scanned. We could consider the
   * slots bound for upper and lower ranges separately, or we could calculate
   * the bytes scanned between the start and stop row of each table.
   *
  ...
  private List<QueryPlan> orderPlansBestToWorst(SelectStatement select, List<QueryPlan> plans, boolean stopAtBestPlan) {
    ...
    /**
     * If we have a plan(s) that are just point lookups (i.e. fully qualified
     * row keys), then favor those first.
     */
     ...
     * there's a limit on the query, we might choose a different plan. For
     * example if the limit was a very large number and the combination of
     * applying other filters on the row key are estimated to choose fewer rows,
     * we'd choose that one.
     */
