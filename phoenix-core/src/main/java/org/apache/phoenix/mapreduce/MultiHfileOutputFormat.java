...
/**
 * The MultiHfileOutputFormat class simplifies writing HFiles for multiple
 * tables. It has been adapted from {#link HFileOutputFormat2} but differs from
 * the fact it creates HFiles for multiple tables.
 */
public class MultiHfileOutputFormat extends FileOutputFormat<TableRowkeyPair, Cell> {
  ...
  /**
   * Returns the set of configurations that have been configured for the table
  ...
  /**
   * Runs inside the task to deserialize column family to bloom filter type map
  ...
  /**
   * Runs inside the task to deserialize column family to block size map from
   * the configuration.
   *
   * @param conf to read the serialized values from
