...
public class Sequence {
  ...
  public enum ValueOp {
    VALIDATE_SEQUENCE, INCREMENT_SEQUENCE
  };

  public enum MetaOp {
    CREATE_SEQUENCE, DROP_SEQUENCE, RETURN_SEQUENCE
  };
  ...
  /**
   * This method first checks whether value.currentValue = value.nextValue, this
   * check is what determines whether we need to refresh the cache when
   * evaluating NEXT VALUE FOR. Once current value reaches the next value we
   * know the cache is exhausted as we give sequence values out one at time.
   *
   * However for bulk allocations, evaluated by NEXT <n> VALUE FOR, we need a
   * different check
   *
   * @see isSequenceCacheExhaustedForBulkAllocation
   *
   * Using the bulk allocation method for determining if the cache is exhausted
   * for both cases works in most of the cases, however when dealing with CYCLEs
   * and overflow and underflow, things break down due to things like sign
   * changes that can happen if we overflow from a positive to a negative number
   * and vice versa. Therefore, leaving both checks in place.
   *
   */
  ...
  /**
   * @throws SQLException with the correct error code if sequence limit is
   * reached with this request for allocation or we attempt to perform a bulk
   * allocation on a sequence with cycles.
   */
   ...
   *
   * @param kv an empty KeyValue used only to get the column family and column
   * qualifier of the key value to be returned (if the sequence row is from a
   * previous version)
   * @param cellIndex index of the KeyValue to be returned (if the sequence row
