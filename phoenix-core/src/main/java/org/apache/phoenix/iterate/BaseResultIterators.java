...
public abstract class BaseResultIterators extends ExplainTable implements ResultIterators {
  ...
  /**
   * Get parallel scans of the specified scan boundaries. This can be used for
   * getting parallel scans when there is split/merges while scanning a chunk.
   * In this case we need not go by all the regions or guideposts.
   *
   * @param scan
   * @return
   * @throws SQLException
   ...
   * scans may be concatenated together directly, while the other ones may need
   * to be merge sorted, depending on the query.
   *
  ...
  /**
   * Executes the scan in parallel across all regions, blocking until all scans
   * are complete.
   *
