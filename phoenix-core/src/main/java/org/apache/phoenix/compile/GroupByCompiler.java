...
/**
 *
 * Validates GROUP BY clause and builds a {@link GroupBy} instance to
 * encapsulate the group by expressions.
 *
 *
 * @since 0.1
 */
public class GroupByCompiler {
   ...
   * @throws AmbiguousColumnException if an unaliased column name is ambiguous
  ...
  public static GroupBy compile(StatementContext context, SelectStatement statement, TupleProjector tupleProjector, boolean isInRowKeyOrder) throws SQLException {
    ...
    /**
     * Distinct can use an aggregate plan if there's no group by. Otherwise, we
     * need to insert a step after the Merge that dedups. Order by only allowed
     * on columns in the select distinct
     */
