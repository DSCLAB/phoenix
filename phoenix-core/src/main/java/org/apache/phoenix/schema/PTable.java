...
public interface PTable extends PMetaDataEntity {
  ...
  /**
   * @return A list of the column families of this table ordered by position.
   */
   ...
   * @throws ColumnNotFoundException if no column with the given name can be
   * found
   ...
   * @param name the column name
   * @return the PColumn with the given name
   * @throws ColumnNotFoundException if no PK column with the given name can be
   * found
  ...
  /**
   * Creates a new row at the specified timestamp using the key for the PK
   * values (from {@link #newKey(ImmutableBytesWritable, byte[][])} and the
   * optional key values specified using values.
   *
   ...
   * @return the new row. Use
   * {@link org.apache.phoenix.schema.PRow#toRowMutations()} to generate the Row
   * to send to the HBase server.
   ...
   * {@link #newKey(ImmutableBytesWritable, byte[][])} and the optional key
   * values specified using values. The timestamp of the key value will be set
   * by the HBase server.
   *
   * @param key the row key of the key value
   * @param values the optional key values
  ...
  /**
   * Formulates a row key using the values provided. The values must be in the
   * same order as {@link #getPKColumns()}.
   *
   * @param key bytes pointer that will be filled in with the row key
   * @param values the PK column values
   * @return the number of values that were used from values to set the row key
   */
  ...
  /**
   * Return the number of buckets used by this table for salting. If the table
   * does not use salting, returns null.
   *
   * @return number of buckets used by this table for salting, or null if
   * salting is not used.
   */
  ...
  /**
   * @return the full name of the parent view for a view or data table for an
   * index table or null if this is not a view or index table. Also returns null
   * for a view of a data table (use @getPhysicalName for this case)
   */
  PName getParentName();

  /**
   * @return the table name of the parent view for a view or data table for an
   * index table or null if this is not a view or index table. Also returns null
   * for a view of a data table (use @getPhysicalTableName for this case)
   */
  PName getParentTableName();

  /**
   * @return the schema name of the parent view for a view or data table for an
   * index table or null if this is not a view or index table. Also returns null
   * for view of a data table (use @getPhysicalSchemaName for this case)
   */
  PName getParentSchemaName();

  /**
   * For a view, return the name of table in Phoenix that physically stores
   * data. Currently a single name, but when views are allowed over multiple
   * tables, will become multi-valued.
   *
   ...
   * in-place when the optimizer tells us it's possible. This is due to
   * PHOENIX-2067 and only applicable for tables using DESC primary key
   * column(s) which have not been upgraded.
   *
  ...
  /**
   * @return Position of the column with {@link PColumn#isRowTimestamp()} as
   * true. -1 if there is no such column.
   */
  ...
  /**
   * @return The sequence name used to get the unique identifier for views that
   * are automatically partitioned.
   */
  String getAutoPartitionSeqName();

  /**
   * @return true if the you can only add (and never delete) columns to the
   * table, you are also not allowed to delete the table
   */
