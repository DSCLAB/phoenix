...
public class MutationState implements SQLCloseable {
  ...
  /**
   * Commit a write fence when creating an index so that we can detect when a
   * data table transaction is started before the create index but completes
   * after it. In this case, we need to rerun the data table transaction after
   * the index creation so that the index rows are generated. See
   * {@link #addDMLFence(PTable)} and TEPHRA-157 for more information.
   *
   ...
   * starting. These entries will not conflict with each other, but they will
   * conflict with a DDL operation of creating an index. See
   * {@link #addDMLFence(PTable)} and TEPHRA-157 for more information.
   *
  ...
  /**
   * Combine a newer mutation with this one, where in the event of overlaps, the
   * newer one will take precedence. Combine any metrics collected for the newer
   * mutation.
   *
  ...
  private Iterator<Pair<PName, List<Mutation>>> addRowMutations(final TableRef tableRef, final Map<ImmutableBytesPtr, RowMutationState> values,
    ...
    final Iterator<PTable> indexes
            = // Only maintain tables with immutable rows through this client-side mechanism
            includeAllIndexes || table.isWALDisabled()
            ? // TODO: remove check for isWALDisabled once PHOENIX-3137 is fixed.
  ...
  /**
   * Get the unsorted list of HBase mutations for the tables with uncommitted
   * data.
   *
  ...
  /**
   * Validates that the meta data is valid against the server meta data if we
   * haven't yet done so. Otherwise, for every UPSERT VALUES call, we'd need to
   * hit the server to see if the meta data has changed.
   *
   * @param connection
  ...
  private class MetaDataAwareHTable extends DelegateHTable {
    ...
    /**
     * Called by Tephra when a transaction is aborted. We have this wrapper so
     * that we get an opportunity to attach our index meta data to the mutations
     * such that we can also undo the index mutations.
     */
    @Override
  ...
  /**
   * Determines whether indexes were added to mutated tables while the
   ...
   * prior to running a query. In this way, they are visible to subsequent reads
   * but are not actually committed until commit is called.
   *
   * @param tableRefs
   * @return true if any data was sent and false otherwise.
   * @throws SQLException
