...
/**
 * Like the {@link ParallelWriterIndexCommitter}, but blocks until all writes
 * have attempted to allow the caller to retrieve the failed and succeeded index
 * updates. Therefore, this class will be a lot slower, in the face of failures,
 * when compared to the {@link ParallelWriterIndexCommitter} (though as fast for
 * writes), so it should be used only when you need to at least attempt all
 * writes and know their result; for instance, this is fine for doing WAL
 * recovery - it's not a performance intensive situation and we want to limit
 * the the edits we need to retry.
 * <p>
 * On failure to {@link #write(Multimap)}, we return a
 * {@link MultiIndexWriteFailureException} that contains the list of
 ...
 * <ol>
 * <li><tt>this</tt> is {@link #stop(String) stopped} or aborted (via the passed
 * {@link Abortable}. This causing any pending tasks to fail whatever they are
 * doing as fast as possible. Any writes that have not begun are not even
 * attempted and marked as failures.</li>
 * <li>A batch write fails. This is the generic HBase write failure - it may
 * occur because the index table is not available, .META. or -ROOT- is
 * unavailable, or any other (of many) possible HBase exceptions.</li>
 * </ol>
 * Regardless of how the write fails, we still wait for all writes to complete
 * before passing the failure back to the client.
 */
  ...
  public void write(Multimap<HTableInterfaceReference, Mutation> toWrite) throws MultiIndexWriteFailureException {
    ...
    for (Entry<HTableInterfaceReference, Collection<Mutation>> entry : entries) {
      ...
      tasks.add(new Task<Boolean>() {
        ...
        /**
         * Do the actual write to the primary table. We don't need to worry
         * about closing the table because that is handled the
         * {@link CachingHTableFactory}.
         */
