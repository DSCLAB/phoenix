...
public class ScanRanges {
  ...
  /**
   * Get the minMaxRange that is applied in addition to the scan range. Only
   * used by the ExplainTable to generate the explain plan.
   */
   ...
   * upperExclusiveKey intersects with the scan ranges and false otherwise. We
   * cannot pass in a KeyRange here, because the underlying compare functions
   * expect lower inclusive and upper exclusive keys. We cannot get their next
   * key because the key must conform to the row key schema and if a null byte
   * is added to a lower inclusive key, it's no longer a valid, real key.
   *
  ...
  /**
   * Use SkipScanFilter under two circumstances: 1) If we have multiple ranges
   * for a given key slot (use of IN) 2) If we have a range (i.e. not a
   * single/point key) that is not the last key slot
   */
