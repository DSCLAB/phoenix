...
public class PhoenixIndexFailurePolicy extends DelegateIndexFailurePolicy {
  ...
  /**
   * Attempt to disable the index table when we can't write to it, preventing
   * future updates until the index is brought up to date, but allowing
   * historical reads to continue until then.
   * <p>
   * In the case that we cannot reach the metadata information, we will fall
   * back to the default policy and kill this server, so we can attempt to
   * replay the edits on restart.
   * </p>
   *
   * @param attempted the mutations that were attempted to be written and the
   * tables to which they were written
  ...
  public void handleFailure(Multimap<HTableInterfaceReference, Mutation> attempted, Exception cause) throws IOException {
    ...
    try {
      ...
      if (!throwing) {
        throw ServerUtil.createIOException(null, cause);
      }
    }
  }
