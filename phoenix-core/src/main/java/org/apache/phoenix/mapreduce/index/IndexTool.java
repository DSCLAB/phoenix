...
public class IndexTool extends Configured implements Tool {
  ...
  /**
   * Uses the HBase Front Door Api to write to index table. Submits the job and
   * either returns or waits for the job completion based on runForeground
   * parameter.
   *
   * @param job
   * @param outputPath
   * @param runForeground - if true, waits for job completion, else submits and
   * returns immediately.
   * @return
   * @throws Exception
   */
