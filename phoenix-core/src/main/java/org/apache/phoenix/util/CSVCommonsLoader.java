...
public class CSVCommonsLoader {
  ...
  /**
   * default settings delimiter = ',' quoteChar = '"', escape = null
   * recordSeparator = CRLF, CR, or LF ignore empty lines allows the last data
   * line to have a recordSeparator
   *
  ...
  /**
   * Translate a field separator, escape character, or phrase delimiter into a
   * control character if it is a single digit other than 0.
   *
   * @param delimiter
   * @return
   */
   ...
   *
   * Data is batched up based on connection batch size. Column PDataType is read
   * from metadata and is used to convert column value to correct type before
   * upsert.
   *
  ...
  static class CsvUpsertListener implements CsvUpsertExecutor.UpsertListener {
    ...
    /**
     * Get the total number of upserts that this listener has been notified
     * about up until now.
     *
