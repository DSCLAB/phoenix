 ...
 * of driver. Supported metadata methods include: null {@link #getTables(String, String, String, String[])}
 ...
 * {@link #getCatalogs()} Other ResultSet methods return an empty result set.
 *
 *
 * @since 0.1
 */
public class PhoenixDatabaseMetaData implements DatabaseMetaData, org.apache.phoenix.jdbc.Jdbc7Shim.DatabaseMetaData {
  ...
  public ResultSet getColumns(String catalog, String schemaPattern, String tableNamePattern, String columnNamePattern)
    ...
    StringBuilder buf = new StringBuilder("select \n "
            ...
            + COLUMN_FAMILY + ","
            + DATA_TYPE + " " + TYPE_ID + ","
            +// raw type id for potential internal consumption
            VIEW_CONSTANT + ","
            + MULTI_TENANT + ","
   ...
   * where each row is a column definition). The tenant id is by definition the
   * first column of the primary key, but the primary key does not necessarily
  ...
  /**
   * We support either: 1) A non null tableNamePattern to find an exactly match
   * with a table name, in which case either a single row would be returned in
   * the ResultSet (if found) or no rows would be returned (if not found). 2) A
   * null tableNamePattern, in which case the ResultSet returned would have one
   * row per table. Note that catalog and schemaPattern must be null or an empty
   * string and types must be null or "TABLE". Otherwise, no rows will be
   * returned.
   */
  @Override
