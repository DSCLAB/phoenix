...
public class PhoenixConfigurationUtilTest extends BaseConnectionlessQueryTest {
   ...
   *
   * When invoking PhoenixConfigurationUtil.getUpsertStatement(), if upserting
   * into a Phoenix View and the View DDL had recently been issued such that
   * MetdataClient cache had been updated as a result of the create table versus
   * from data in SYSTEM.CATALOG, the Upsert statement would contain the
   * Object.toString() classname + hashcode instead of the correct
   * cf.column_name representation which would cause the calling Pig script to
   * fail.
   */
