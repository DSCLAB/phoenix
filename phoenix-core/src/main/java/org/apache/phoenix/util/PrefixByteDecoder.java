...
public class PrefixByteDecoder {
  ...
  /**
   * Used when the maximum length of encoded byte array is not known. Will cause
   * a new byte array to be allocated for each call to
   * {@link #decode(DataInput)}.
   */
   ...
   * Will not allocate new byte array with each call to
   * {@link #decode(DataInput)}.
   *
   * @param maxLength maximum length needed for any call to
   * {@link #decode(DataInput)}.
   */
  ...
  /**
   * Resets state of decoder if it will be used to decode bytes from a different
   * DataInput.
   */
  public void reset() {
   ...
   * @return Pointer containing bytes that were decoded. Note that the same
   * pointer will be returned with each call, so it must be consumed prior to
