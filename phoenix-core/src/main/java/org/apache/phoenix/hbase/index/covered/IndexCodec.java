 ...
 * <p>
 * Generally, you should extend {@link BaseIndexCodec} instead, so help maintain
 * compatibility as features need to be added to the codec, as well as
 * potentially not haivng to implement some methods.
 */
public interface IndexCodec {
  ...
  /**
   * Get the index cleanup entries. Currently, this must return just single row
   * deletes (where just the row-key is specified and no columns are returned)
   * mapped to the table name. For instance, to you have an index 'myIndex' with
   * row :
   *
   * <pre>
   ...
   * Generally, you only care about the latest column values, for each column
   * you are indexing for each index table.
   ...
   * The returned {@link Put}s need to be fully specified (including timestamp)
   * to minimize passes over the same key-values multiple times.
   * <p>
   * You must specify the same timestamps on the Put as
   * {@link TableState#getCurrentTimestamp()} so the index entries match the
   * primary table row. This could be managed at a higher level, but would
   * require iterating all the kvs in the Put again - very inefficient when
   * compared to the current interface where you must provide a timestamp
   ...
   * table will take place in a batch of updates; if we know that the mutation
   * is part of a batch, we can build the state much more intelligently.
   * <p>
   * <b>If you have batches that have multiple updates to the same row state,
   * you must specify a batch id for each batch. Otherwise, we cannot guarantee
   * index correctness</b>
   *
