 ...
 * column index for a SELECT clause projection. The column index may be looked
 * up via the name using {@link #getColumnIndex(String)}.
 *
 *
 * @since 0.1
 */
public class RowProjector {
   ...
   *
   * @param columnProjectors ordered list of ColumnProjectors corresponding to
   * projected columns in SELECT clause aggregating coprocessor. Only required
   * in the case of an aggregate query with a limit clause and otherwise may be
   * null.
