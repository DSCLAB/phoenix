...
public class RoundFloorCeilExpressionsTest {
  ...
  private static enum RoundingType {
    ...
    /**
     * Returns a rounding expression of this type that will round the given
     * decimal key at the given scale.
     *
  ...
  /**
   * Represents a possible relational operator used in rounding expression where
   * clauses. Includes information not kept by CompareFilter.CompareOp,
   * including a string symbol representation and a method for actually
  ...
  /**
   * Produces a string error message containing the given information, formatted
   * like a where clause. <br>
   * Example Output: <br>
  ...
  /**
   * Interpreting the KeyRange as a range of decimal, produces a nicely
  ...
  /**
   * Checks that a given KeyPart produces the right key ranges for each
  ...
  /**
   * Checks that a given KeyRange's boundaries match with the given rounding
   * expression type, rounding scale, relational operator, and right hand side
   * decimal. Does so by checking the decimal values immediately on either side
   * of the KeyRange border and verifying that they either match or do not match
   * the "where clause" formed by the rounding type, scale, relation, and rhs
   * decimal. If a relation should produce an unbounded upper or lower range,
   * verifies that that end of the range is unbounded. Finally, if the range is
   * empty, verifies that the rhs decimal required more precision than could be
  ...
  /**
   * Produces the previous Decimal key relative to the given key. The new key
   * will differ from the old key in as small a unit as possible while still
   * maintaining accurate serialization.
   *
   * @param key bytes for the old Decimal key
   * @return bytes for the new Decimal key, a single unit previous to the old
   * one
   */
  ...
  /**
   * Produces the next Decimal key relative to the given key. The new key will
   * differ from the old key in as small a unit as possible while still
   * maintaining accurate serialization.
   *
   * @param key bytes for the old Decimal key
  ...
  /**
   * Produces the smallest unit of difference possible for the given decimal
   * that will still be serialized accurately. For example, if the
   * MAXIMUM_RELIABLE_PRECISION were 4, then getSmallestUnit(2.3) would produce
   * 0.001, as 2.301 could be serialized accurately but 2.3001 could not.
   *
   * @param decimal the decimal to find the smallest unit in relation to
   * @return the smallest BigDecimal unit possible to add to decimal while still
   * maintaining accurate serialization
   * @throws IllegalArgumentException if decimal requires more than the maximum
   * reliable precision
   */
