...
public class QueryOptimizer {
   ...
   * query is a point lookup (i.e. we have a set of exact row keys), choose
   * among those. 2) If the query has an ORDER BY and a LIMIT, choose the plan
   * that has all the ORDER BY expression in the same order as the row key
   ...
   * start/stop scan key. b) the plan that preserves ordering for a group by. c)
   * the data table plan
   *
