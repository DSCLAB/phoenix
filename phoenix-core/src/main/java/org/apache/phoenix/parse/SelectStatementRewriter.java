...
/**
 *
 * Class that creates a new select statement by filtering out nodes. Currently
 * only supports filtering out boolean nodes (i.e. nodes that may be ANDed and
 * ORed together.
 *
 * TODO: generize this
 *
...
public class SelectStatementRewriter extends ParseNodeRewriter {
  ...
  /**
   * Rewrite the select statement by filtering out expression nodes from the
   * WHERE clause
   *
  ...
  /**
   * Rewrite the select statement by filtering out expression nodes from the
   * HAVING clause and anding them with the WHERE clause.
   *
   * @param statement the select statement from which to move the nodes.
   * @param moveNodes expression nodes to filter out of HAVING clause and add to
   * WHERE clause.
   * @return new select statement
   * @throws SQLException
   */
