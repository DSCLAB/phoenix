...
public class IndexHalfStoreFileReader extends StoreFile.Reader {
  ...
  public HFileScanner getScanner(final boolean cacheBlocks, final boolean pread,
    ...
    return new HFileScanner() {
      ...
      public boolean seekBefore(byte[] key, int offset, int length) throws IOException {
        ...
        } else // The equals sign isn't strictly necessary just here to be consistent with
        // seekTo
        {
      ...
      public int seekTo(byte[] key, int offset, int length) throws IOException {
        if (top) {
           ...
           */
        } else if (getComparator().compare(key, offset, length, splitkey, 0, splitkey.length) >= 0) {
  ...
  /**
   * In case of top half store, the passed key will be with the start key of the
   * daughter region. But in the actual HFiles, the key will be with the start
   * key of the old parent region. In order to make the real seek in the HFiles,
   * we need to build the old key.
   *
   * The logic here is just replace daughter region start key with parent region
   * start key in the key part.
   *
   * @param key
   *
   */
