...
/**
 *
 * Interface to group together services needed during querying. The parameters
 * that may be set in {@link org.apache.hadoop.conf.Configuration} are
 * documented here: https://github.com/forcedotcom/phoenix/wiki/Tuning
 *
 *
 * @since 0.1
 */
...
public interface QueryServices extends SQLCloseable {
   ...
   * <p>
   * if the threshold is reached, a {@link SpoolTooBigToDiskException } will be
   * thrown
   */
  ...
  /**
   * Get the properties from the HBase configuration in a read-only structure
   * that avoids any synchronization
   */
