...
public class GroupedAggregateRegionObserver extends BaseScannerRegionObserver {
   ...
   * formed by the list of expressions from the scan and returns the aggregated
   * rows of each group. For example, given the following original rows in the
   * RegionScanner: KEY COL1 row1 a row2 b row3 a row4 a the following rows will
   * be returned for COUNT(*): KEY COUNT a 3 b 1 The client is required to do a
   * sort and a final aggregation, since multiple rows with the same key may be
   * returned from different regions.
   */
  @Override
   ...
   * GROUPBY_SPILLABLE_ATTRIB is set to false. The memory usage is tracked at a
   * coursed grain and will throw and abort if too much is used.
   *
   *
   * @since 3.0.0
   */
