...
/**
 *
 * Child memory manager that delegates through to global memory manager, but
 * enforces that at most a threshold percentage is used by this memory manager.
 * No blocking is done if the threshold is exceeded, but the standard blocking
 * will be done by the global memory manager.
 *
 *
 * @since 0.1
 */
