...
/**
 * Write the metrics to a phoenix table. Generally, this class is instantiated
 * via hadoop-metrics2 property files. Specifically, you would create this class
 * by adding the following to by This would actually be set as: <code>
 ...
 * </ol>
 * and
 * <tt>some instance name</tt> is just any unique name, so properties can be
 * differentiated if there are multiple sinks of the same type created
 */
public class PhoenixMetricsSink implements MetricsSink {
   ...
   * when creating upsert statements
   *
