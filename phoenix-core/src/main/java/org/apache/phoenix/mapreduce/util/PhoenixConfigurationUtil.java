...
/**
 * A utility class to set properties on the {#link Configuration} instance. Used
 * as part of Map Reduce job configuration.
 *
 */
public final class PhoenixConfigurationUtil {
   ...
   *
   * @param configuration
   * @param quorum ZooKeeper quorum string for HBase cluster the MapReduce job
   * will read from
   */
  ...
  public static List<ColumnInfo> getUpsertColumnMetadataList(final Configuration configuration) throws SQLException {
    ...
    if (isNotEmpty(upsertColumns)) {
      ...
      LOG.info(String.format("UseUpsertColumns=%s, upsertColumns=%s, upsertColumnSet.size()=%s, parsedColumns=%s ", !upsertColumnList.isEmpty(), upsertColumns, upsertColumnList.size(), Joiner.on(DEFAULT_COLUMN_NAMES_DELIMITER).join(upsertColumnList)
      ));
    }
  ...
  private static List<String> getSelectColumnList(
    ...
    if (isNotEmpty(selectColumns)) {
      ...
      LOG.info(String.format("UseSelectColumns=%s, selectColumns=%s, selectColumnSet.size()=%s, parsedColumns=%s ", !selectColumnList.isEmpty(), selectColumns, selectColumnList.size(), Joiner.on(DEFAULT_COLUMN_NAMES_DELIMITER).join(selectColumnList)
      ));
    }
