...
/**
 *
 * JDBC ResultSetMetaData implementation of Phoenix. Currently only the
 * following methods are supported: - {@link #getColumnCount()}
 * - {@link #getColumnDisplaySize(int)}
 * - {@link #getColumnLabel(int)} displays alias name if present and column name
 * otherwise - {@link #getColumnName(int)} same as {@link #getColumnLabel(int)}
 ...
 * {@link #getCatalogName(int)} always returns empty string - {@link #isNullable(int)}
 * - {@link #isSigned(int)}
 * - {@link #isAutoIncrement(int)} always false - {@link #isCurrency(int)}
 * always false - {@link #isDefinitelyWritable(int)} always false -
 * {@link #isReadOnly(int)} always true - {@link #isSearchable(int)} always true
 *
 *
 * @since 0.1
 */
